"""
主体名称识别节点（Item Name Recognition Node）

这个节点负责从整份文档中提取“这份资料主要在讲哪个产品/型号/设备”。

它在导入链里的战略意义很大：
- 没有这一步，查询侧很难先做商品名确认；
- 有了这一步，查询时可以先锁定产品，再在该产品文档内做正文检索。

阅读重点：
1. `item_name` 是如何从文件标题和正文中抽取出来的；
2. 为什么要把它单独写成文档级索引；
3. 为什么还要把它回填到每个 chunk 上。
"""
...
# 这类词更像文档体裁，不像真正的设备主体名称。
GENERIC_ITEM_NAME_PATTERNS = (
    "说明书",
    "使用说明",
    "用户手册",
    "安装指南",
...
# 常见型号通常混合字母和数字，必要时带空格或连接符。
MODEL_NAME_PATTERN = re.compile(
    r"\b(?:[A-Za-z]+[\- ]*){0,3}[A-Za-z]{2,}[A-Za-z0-9\- ]{1,20}\d[A-Za-z0-9\- ]{0,12}\b"
)
...
def _is_valid_item_name(text: str) -> bool:
    ...
    if not stripped:
        return False
    if stripped.lower() in INVALID_ITEM_NAMES:
        return False
    if stripped in INVALID_ITEM_NAMES:
        return False
    if len(stripped) < 2 or len(stripped) > 80:
        return False
    if not re.search(r"[\u4e00-\u9fffA-Za-z0-9]", stripped):
        return False
    if any(pattern in stripped for pattern in GENERIC_ITEM_NAME_PATTERNS):
        return False
    return True


def _extract_anchor_name(file_title: str, chunks: List[Dict]) -> str:
    candidates: List[str] = []

    for chunk in chunks[:5]:
        if not isinstance(chunk, dict):
            continue
        title = (chunk.get("title") or "").strip().lstrip("#").strip()
        if title and title not in GENERIC_SECTION_TITLES:
            candidates.append(title)
        candidates.extend(_extract_candidates_from_chunk(chunk))

    candidates.append((file_title or "").strip())

    for candidate in candidates:
        if _is_valid_item_name(candidate):
            return candidate
    return (file_title or "").strip()


def _extract_candidates_from_chunk(chunk: Dict[str, Any]) -> List[str]:
    """
    从正文中抽取更像设备名/型号的候选词。

    之前只看标题，很多手册首屏标题只有“使用说明书”，真正型号在正文第一段。
    这里补一层正文候选，优先把“Brother HAK 180 / HAK180”这类实体提上来。
    """
    if not isinstance(chunk, dict):
        return []
    ...
    # 优先找品牌+型号组合，例如 Brother HAK 180。
    for match in re.finditer(r"\b([A-Z][A-Za-z]+(?:\s+[A-Z][A-Za-z0-9]+){1,4})\b", compact_content):
        candidate = _clean_item_name_candidate(match.group(1))
        if _looks_like_specific_model(candidate):
            candidates.append(candidate)

    # 再兜底找纯型号，例如 HAK180 / HL-3040。
    for match in MODEL_NAME_PATTERN.finditer(compact_content):
        candidate = _clean_item_name_candidate(match.group(0))
        if _looks_like_specific_model(candidate):
            candidates.append(candidate)
...
def _clean_item_name_candidate(text: str) -> str:
    return re.sub(r"\s+", " ", (text or "").strip(" ,.;:()[]{}<>#*")).strip()


def _looks_like_specific_model(text: str) -> bool:
    candidate = (text or "").strip()
    if not candidate:
        return False

    normalized = _normalize_item_name(candidate)
    if any(pattern in candidate for pattern in GENERIC_ITEM_NAME_PATTERNS):
        return False
    ...
    # 兜底允许明显的品牌+系列写法，例如 Brother XXX。
    return len(normalized) >= 8 and "brother" in normalized


def _share_meaningful_overlap(left: str, right: str) -> bool:
    left_norm = _normalize_item_name(left)
    right_norm = _normalize_item_name(right)
    ...
    overlap_ratio = len(left_chars & right_chars) / min(len(left_chars), len(right_chars))
    return overlap_ratio >= 0.6


def _finalize_item_name(file_title: str, anchor_name: str, context: str, item_name: str) -> str:
    fallback_name = anchor_name or file_title
    candidate = (item_name or "").strip()
    context_norm = _normalize_item_name(context)
    candidate_norm = _normalize_item_name(candidate)
    fallback_norm = _normalize_item_name(fallback_name)

    if not _is_valid_item_name(candidate):
        logger.warning(f"步骤3：识别结果无效，回退到锚点名称：{fallback_name}")
        return fallback_name

    if candidate in context or (candidate_norm and candidate_norm in context_norm):
        return candidate

    if candidate_norm and fallback_norm and candidate_norm != fallback_norm:
        if fallback_norm in candidate_norm or candidate_norm in fallback_norm:
            logger.warning(
                f"步骤3：识别结果[{candidate}]是锚点[{fallback_name}]的扩写，但正文未出现，回退为锚点名称"
            )
            return fallback_name

    if fallback_name and _share_meaningful_overlap(candidate, fallback_name) and len(candidate_norm) <= len(fallback_norm) + 2:
        return candidate

    logger.warning(
        f"步骤3：识别结果[{candidate}]未在正文出现，且与锚点[{fallback_name}]缺乏重合，回退为锚点名称"
    )
    return fallback_name
...
def node_item_name_recognition(state: ImportGraphState) -> ImportGraphState:
    ...
    try:
        ...
        if not chunks:
            ...
            return state
        anchor_name = _extract_anchor_name(file_title, chunks)
        ...
        item_name = step_3_call_llm(file_title, context, anchor_name)
...
def step_3_call_llm(file_title: str, context: str, anchor_name: str) -> str:
    """
    ...
    if not context:
        ...
        return anchor_name or file_title
    ...
    try:
        ...
        final_item_name = _finalize_item_name(file_title, anchor_name, context, item_name)
        logger.info(f"步骤3：大模型识别商品名称成功，结果为：{final_item_name}")
...
# ──────────────────────────────────────────────────────────
# 阅读导航
# 下一篇: app/import_process/agent/nodes/node_bge_embedding.py
# 总导航: doc/代码阅读顺序.md
# ──────────────────────────────────────────────────────────
