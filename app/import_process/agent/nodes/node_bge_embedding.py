"""
BGE 向量化节点（BGE Embedding Node）

这个节点把上游产出的 chunk 文本转换成可检索向量，是“内容加工”到“检索资产”
之间的桥梁。向量化完成后，文本才具备进入 Milvus 的基础条件。

阅读重点：
1. 为什么要同时生成稠密向量和稀疏向量；
2. 为什么要把 `item_name` 和正文内容拼在一起再向量化；
3. 为什么采用分批处理而不是一次性全部送入模型。
"""
...
# ──────────────────────────────────────────────────────────
# 阅读导航
# 下一篇: app/import_process/agent/nodes/node_import_milvus.py
# 总导航: doc/代码阅读顺序.md
# ──────────────────────────────────────────────────────────
