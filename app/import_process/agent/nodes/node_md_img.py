"""
Markdown 图片处理节点（Markdown Image Node）

这个节点解决一个常见问题：原始 Markdown 里的图片通常只是“路径引用”，
而不是可直接检索的文本信息。为了让图片内容也能进入后续检索链，它会：

1. 找出 Markdown 中的图片引用；
2. 读取或上传图片资源；
3. 通过多模态能力生成图片语义描述；
4. 把描述信息重新写回 Markdown。

因此，这个节点本质上是在把“图像信息”转换成“可检索文本”。
"""
...
def step_4_upload_and_replace(minio_client: Minio, doc_stem: str, targets: List[Tuple[str, str, Tuple[str, str]]],
    ...
    minio_img_dir = (minio_config.minio_img_dir or "").strip().strip("/")
    sanitized_doc_stem = doc_stem.replace(" ", "")
    upload_dir = f"{minio_img_dir}/{sanitized_doc_stem}" if minio_img_dir else sanitized_doc_stem
...
def upload_to_minio(minio_client: Minio, local_path: str, object_name: str) -> str | None:
    ...
    try:
        ...
        img_url = f"{base_url}/{quote(object_name, safe='/')}"
...
# ──────────────────────────────────────────────────────────
# 阅读导航
# 下一篇: app/import_process/agent/nodes/node_document_split.py
# 总导航: doc/代码阅读顺序.md
# ──────────────────────────────────────────────────────────
