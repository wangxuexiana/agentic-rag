"""
PDF 转 Markdown 节点（PDF To Markdown Node）

这个节点负责把 PDF 统一转换成 Markdown，目的是给下游提供稳定的文本中间格式。
你可以把它理解为“导入链的格式标准化层”：

- 上游文件可能是 PDF；
- 下游切分、图片处理、主体识别都更适合消费 Markdown；
- 所以这里把不稳定的 PDF 页面结构，先收敛成统一的 `.md` 文件。

阅读重点：
1. 远端解析结果如何下载和落盘；
2. 结果目录如何整理；
3. 最终如何把 `md_path` 回填给状态流转。
"""
...
# 关闭SSL警告：MinerU CDN证书在部分网络环境下验证失败，需跳过验证
urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)

from app.config.mineru_config import MINERU_API_TOKEN, MINERU_BASE_URL, MINERU_BYPASS_PROXY
...
def _create_mineru_session() -> requests.Session:
    """
    统一创建 MinerU 请求会话。

    默认绕过系统代理，避免 VPN/代理软件注入的 HTTP(S)_PROXY 干扰
    MinerU API、signed URL 上传和 CDN 下载。
    """
    session = requests.Session()
    session.trust_env = not MINERU_BYPASS_PROXY
...
def step_2_upload_and_poll(pdf_path_obj: Path, output_dir_obj: Path):
    ...
    session = _create_mineru_session()
    logger.info(
        f"[MinerU网络] bypass_proxy={MINERU_BYPASS_PROXY}，将"
        f"{'绕过' if MINERU_BYPASS_PROXY else '继承'}系统代理"
    )
    ...
    resp = session.post(url=url_get_upload, headers=request_headers, json=req_data, timeout=30)
    ...
    try:
        # 首次上传：自动识别文件类型
        put_resp = session.put(url=signed_url, data=file_data, timeout=60)
        ...
        if put_resp.status_code != 200:
            ...
            put_resp = session.put(url=signed_url, data=file_data, headers=pdf_headers, timeout=60)
            # 重试仍失败则抛出异常
            if put_resp.status_code != 200:
    ...
    try:
        while True:
            ...
            # 发起轮询请求，短超时10秒，异常则重试
            try:
                poll_resp = session.get(url=poll_url, headers=request_headers, timeout=10)
            except Exception as e:
...
def step_3_download_and_extract(zip_url: str, output_dir_obj: Path, pdf_stem: str) -> str:
    ...
    session = _create_mineru_session()
    try:
        resp = session.get(zip_url, timeout=120, verify=False)
        if resp.status_code != 200:
...
# ──────────────────────────────────────────────────────────
# 阅读导航
# 下一篇: app/import_process/agent/nodes/node_md_img.py
# 总导航: doc/代码阅读顺序.md
# ──────────────────────────────────────────────────────────
