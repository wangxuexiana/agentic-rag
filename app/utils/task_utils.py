...
def get_task_result_json(task_id: str, key: str, default: Any = None) -> Any:
    """
    获取 JSON 结果字段。

    说明：
    - 任务结果在内存里统一按字符串保存，便于不同流程共用；
    - 对于 `import_summary` 这类结构化结果，这里负责做一次 JSON 反序列化；
    - 如果字段不存在或解析失败，返回调用方给定的默认值。
    """
    raw_value = get_task_result(task_id, key, "")
    if not raw_value:
        return default
    try:
