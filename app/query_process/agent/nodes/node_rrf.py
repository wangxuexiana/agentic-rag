"""
RRF 倒数排名融合节点 (Reciprocal Rank Fusion Node)

本模块实现 Agentic RAG 查询流程中的「多路检索结果融合」环节。
将来自不同检索源（Embedding 检索、HyDE 检索、知识图谱检索等）的结果，
通过 RRF 算法进行融合排序，消除单路检索的偏差，提升召回质量。

核心算法：
    RRF 公式：score(chunk) = Σ source_weight × (1 / (k + rank))
    - k=60 是经典平滑常数，避免高排名文档占据过大优势
    - source_weight 为不同检索源分配权重（当前 Embedding=1.0, HyDE=1.0）
    - 最终按融合分数降序排列，取 Top10

辅助函数：
- _as_entity_list(): 将上游异构检索结果统一规整为标准字典列表
  兼容 Pymilvus Hit 对象、嵌套 entity 字典、扁平字典等多种格式
"""
...
def _as_entity_list(state_list) -> List[Dict[str, Any]]:
    ...
    # 执行步骤：
    # 1. 统一处理上游可能出现的多种结果结构
    # 2. 提取 entity / id / score 等核心字段
    # 3. 规范化成普通 dict 列表，便于后续 RRF 计算
...
def reciprocal_rank_fusion(
    ...
    # 执行步骤：
    # 1. 遍历每个来源列表及其权重
    # 2. 对每个文档按排名计算 1 / (k + rank) 的贡献分
    # 3. 同一 chunk_id 在多个来源中出现时累加得分
    # 4. 最后按总分排序并截断
...
# ──────────────────────────────────────────────────────────
# 阅读导航
# 下一篇: app/query_process/agent/nodes/node_rerank.py
# 总导航: doc/代码阅读顺序.md
# ──────────────────────────────────────────────────────────
